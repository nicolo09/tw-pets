    ...
    <div class="card-footer w-100 p-0 m-0">
        ...
        <div class="comments text-left m-1">
                ...
                foreach ($templateParams["comments"] as $comment) {
                    ...
                    if (isset($templateParams["son-comments-" . $comment["id_commento"]]) && $templateParams["son-comments-" . $comment["id_commento"]] == true) {
                        ...
                        if (isset($templateParams["canLoadMoreComments"]) && $templateParams["canLoadMoreComments"] == false) {
                            ...
                            echo html_entity_decode('<button id="' . $id . '-son-comment-' . $comment["id_commento"] . '" class="rounded btn btn-outline-primary" onclick="window.location.href=\'view-post-profile.php?id=' . $id . '\';" >Leggi le risposte</button>');
                        } else {
            ...
            if (isset($templateParams["more-comments"]) && $templateParams["more-comments"] == true && isset($templateParams["id"])) {
                ...
                if (isset($templateParams["canLoadMoreComments"]) && $templateParams["canLoadMoreComments"] == false) {
                    //Non posso caricare in questa pagina altri commenti, redirect a pagina post singolo
                    echo html_entity_decode('<button id="' . $id . '-comment-load" class="d-block rounded btn btn-outline-secondary" onclick="window.location.href=\'view-post-profile.php?id=' . $id . '\';">Carica altri commenti</button>');
                } else {
                    echo html_entity_decode('<button id="' . $id . '-comment-load" class="d-block rounded btn btn-outline-secondary">Carica altri commenti</button>');
                }
            }
            ?>
            <!--New comment-->
